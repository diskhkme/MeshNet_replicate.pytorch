...
class MeshNet(nn.Module):
    def __init__(self, cfg, require_fea=False):
        ...
        self.fusion_mlp = nn.Sequential(
            nn.Conv1d(1024, 1024, 1), # input channel is sum of spatal and structure out of second mesh conv layer
            nn.BatchNorm1d(1024),
            nn.ReLU(),
        )
    ...
    def forward(self, centers, corners, normals, neighbor_index):
        ...
        # add skip connection from mesh conv feature output
        # output is a single global feature of a mesh (b, 1024, num_face)
        ...
        fea = self.classifier[:-1](fea) # without last lenear layer to return feature vector(for what?)
...
if __name__ == '__main__':
    import config
    cfg = config.get_train_config('../config/train_config.yaml')
    ...
    m_net = MeshNet(cfg['MeshNet'])
    cls = m_net(centers, corners, normals, neighbor_indices)
